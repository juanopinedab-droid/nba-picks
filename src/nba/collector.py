...
from ..core.utils import http_get, extract_best_odds

# ─── SEASON ACTIVA ─────────────────────────────────────────────────────────────
# Se puede cambiar vía set_season() desde la API o CLI.
# Al cambiar de season, todas las caches se limpian automáticamente.

_active_season = config.NBA_SEASON
...
def set_season(season: str) -> str:
    ...
    global _gamelog_cache, _player_recent_cache

    if season != _active_season:
        _regular_team_cache.clear()
        _playoff_team_cache.clear()
        _regular_home_cache.clear()
        _regular_away_cache.clear()
        _playoff_home_cache.clear()
        _playoff_away_cache.clear()
        _regular_player_cache.clear()
        _playoff_player_cache.clear()
        _gamelog_cache.clear()
        _player_recent_cache.clear()
...
def get_todays_odds() -> list[dict]:
    ...
    for g in resp.json():
        ...
        odds = extract_best_odds(g["bookmakers"], home, away)
        if not odds:
            continue
        games.append({
            "game_id":   g["id"],
