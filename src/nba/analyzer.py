...
if not _USE_LR:
    try:
        ...
        params = _db.get_active_model_params()
        if params and params["model_type"] == "logistic_regression":
            _LR_COEF      = params["params"]["coef"]
            _LR_INTERCEPT = params["params"]["intercept"]
            _LR_MEANS     = params["params"]["means"]
            _LR_SCALES    = params["params"]["scales"]

            def _lr_prob(features: list) -> float:
                scaled = [(f - m) / s for f, m, s in zip(features, _LR_MEANS, _LR_SCALES)]
                z = _LR_INTERCEPT + sum(c * x for c, x in zip(_LR_COEF, scaled))
...
def analyze_game(game: dict, home_stats: dict, away_stats: dict,
    ...
    def _book_agreement(our_prob: float, impl_by_book: list[float]) -> tuple[int, str]:
        ...
        if not impl_by_book:
            return 0, ""
        agree = sum(1 for p in impl_by_book if our_prob - p >= config.get_min_edge())
    ...
    if edge_home >= config.get_min_edge() and our_prob_home >= 0.52 and home_ok:
    ...
    if edge_away >= config.get_min_edge() and our_prob_away >= 0.52 and away_ok:
        picks.append(_build_pick(
            game=game,
            bet_type="MONEYLINE",
...
def _analyze_spread(game: dict, net_diff: float,
    ...
    if spread_diff < -1.5 and our_prob_home > 0.55:
        ...
        if spread_edge >= config.get_min_edge():
            return _build_pick(
                game=game,
                bet_type="SPREAD",
...
def _analyze_total(game: dict, home_stats: dict, away_stats: dict,
    ...
    if our_prob < 0.52 or edge < config.get_min_edge():
        return None
