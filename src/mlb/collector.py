...
_CACHE_FILE    = Path("data") / "mlb_cache.json"
...
def get_park_factor(team_name: str) -> float:
    ...
    name = config.MLB_PARK_ALIASES.get(team_name, team_name)
    return config.MLB_PARK_FACTORS.get(name, 1.00)
...
def get_pitcher_stats(pitcher_id: int, season: int | None = None) -> dict | None:
    ...
    try:
        ...
        fip = ((13 * hr + 3 * (bb + hbp) - 2 * k) / ip + config.MLB_FIP_CONSTANT) if ip >= 5 else era
        ...
        expected_hr = fly_balls * config.MLB_LEAGUE_HR_PER_FB if fly_balls > 0 else hr
        xfip = ((13 * expected_hr + 3 * (bb + hbp) - 2 * k) / ip + config.MLB_FIP_CONSTANT) if ip >= 5 else era
...
def get_pitcher_recent_fip(pitcher_id: int, n_starts: int = 5) -> dict | None:
    ...
    fip_recent = (13 * total_hr + 3 * (total_bb + total_hbp) - 2 * total_k) / total_ip + config.MLB_FIP_CONSTANT
    ...
    expected_hr_recent = total_fly_balls * config.MLB_LEAGUE_HR_PER_FB if total_fly_balls > 0 else total_hr
    xfip_recent = (13 * expected_hr_recent + 3 * (total_bb + total_hbp) - 2 * total_k) / total_ip + config.MLB_FIP_CONSTANT
...
def get_all_team_pitching_stats(season: int | None = None) -> dict[str, dict]:
    ...
    for entry in splits:
        ...
        try:
            ...
            team_fip = ((13 * hr + 3 * (bb + hbp) - 2 * k) / ip + config.MLB_FIP_CONSTANT) if ip >= 1 else era
...
def get_team_recent_pitching(team_name: str, n_days: int = 4) -> dict | None:
    ...
    team_id = config.MLB_TEAM_ID_MAP.get(team_name)
    if not team_id:
        for k, v in config.MLB_TEAM_ID_MAP.items():
...
def get_weather_for_game(home_team: str, commence_iso: str | None = None) -> dict | None:
    ...
    info = config.MLB_STADIUM_INFO.get(home_team)
...
_H2H_ADJ_SCALE  = 4.0    # factor de conversión: 1 OPS unit diff → X carreras ajuste/partido
                          # config.MLB_LEAGUE_AVG_RUNS / config.MLB_LEAGUE_AVG_OPS ≈ 4.45 / 0.718 ≈ 6.2; usamos 4.0
...
def get_pitcher_vs_team_ops(pitcher_id: int, opp_team_id: int) -> dict | None:
    ...
    Args:
        ...
        opp_team_id:  ID del equipo oponente (de config.MLB_TEAM_ID_MAP).
...
def get_savant_team_batting_xwoba(season: int | None = None) -> dict[str, dict]:
    ...
    import csv as _csv
    # Savant usa algunas abreviaturas distintas a nuestro config.MLB_TEAM_MAP
...
def get_todays_mlb_games() -> list[dict]:
    ...
    for odds in odds_list:
        ...
        home_team_id = config.MLB_TEAM_ID_MAP.get(home)
        away_team_id = config.MLB_TEAM_ID_MAP.get(away)
        ...
        away_season_ops_for_h2h = (away_off or {}).get("ops") or config.MLB_LEAGUE_AVG_OPS
        home_season_ops_for_h2h = (home_off or {}).get("ops") or config.MLB_LEAGUE_AVG_OPS
        ...
        away_season_ops  = (away_off or {}).get("ops") or config.MLB_LEAGUE_AVG_OPS
        home_season_ops  = (home_off or {}).get("ops") or config.MLB_LEAGUE_AVG_OPS
        ...
        away_abbr_key = config.MLB_TEAM_MAP.get(away, away[:3].upper())
        home_abbr_key = config.MLB_TEAM_MAP.get(home, home[:3].upper())
        ...
        result.append({
            **odds,
            ...
            "away_abbr":       config.MLB_TEAM_MAP.get(away, away[:3].upper()),
            "home_abbr":       config.MLB_TEAM_MAP.get(home, home[:3].upper()),
