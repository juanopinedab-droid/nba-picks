...
from . import line_tracker
from ..core import config

# ── Parámetros parametrizables del modelo ────────────────────────────────────
from dataclasses import dataclass, field

@dataclass
class ModelParams:
    min_edge: float = 0.07
    min_edge_under: float = 0.08
    min_prob_over: float = 0.55
    min_prob_under: float = 0.60
    max_picks: int = 3
    max_total_line: float = 8.5
    max_pick_edge: float = 0.14
    allow_over: bool = True
    allow_under: bool = False
    allow_runline: bool = True
    allow_moneyline: bool = True
    allow_f5: bool = False
    over_min_mu_margin: float = 1.0
    over_min_weak_xfip: float = 4.2
    elite_ace_xfip: float = 3.85
    elite_ace_shrinkage: float = 0.10
    over_min_recent_rg_ratio: float = 0.87
    under_max_xfip: float = 3.5
    under_min_line: float = 8.0
    confidence_high: float = 0.10
    confidence_med: float = 0.08
    platoon_ops_scale: float = 0.025
    platoon_adj_min: float = 0.003
    temp_baseline_f: float = 72.0
    temp_adj_per_10f: float = 0.50
    wind_adj_per_mph: float = 0.07
    weather_adj_cap: float = 1.8
    rest_adj_short: float = 0.20
    rest_adj_optimal: float = 0.00
    rest_adj_extended: float = 0.10
    rest_adj_long: float = 0.20
    rest_adj_emergency: float = 0.40
    ml_min_edge: float = 0.03
    ml_min_prob: float = 0.60
    ml_max_picks: int = 3
    rl_min_edge: float = 0.04
    rl_min_prob: float = 0.58
    rl_max_picks: int = 3
    min_confidence: str = "BAJA"


_DEFAULT_PARAMS = ModelParams()
...
def expected_runs_team(
    ...
    Fórmula:
        offense_mult  = team_ops / config.MLB_LEAGUE_AVG_OPS
        pitching_mult = pitcher_fip / config.MLB_LEAGUE_AVG_FIP   (FIP alto = más carreras)
        home_adj      = 1.04 si local, 0.97 si visitante
        raw           = config.MLB_LEAGUE_AVG_RUNS × offense × pitching × park × home_adj
        shrunk        = config.MLB_LEAGUE_AVG_RUNS + (raw - config.MLB_LEAGUE_AVG_RUNS) × _SHRINKAGE
    ...
    offense_mult  = team_ops / config.MLB_LEAGUE_AVG_OPS
    pitching_mult = fip_capped / config.MLB_LEAGUE_AVG_FIP
    home_adj      = 1.04 if is_home else 0.97

    raw = config.MLB_LEAGUE_AVG_RUNS * offense_mult * pitching_mult * park_factor * home_adj
    # Shrink hacia el promedio de liga per-equipo
    return config.MLB_LEAGUE_AVG_RUNS + (raw - config.MLB_LEAGUE_AVG_RUNS) * _SHRINKAGE


def p_over_under(
    mu_total: float,
    line: float,
    sigma: float = config.MLB_RUN_TOTAL_SIGMA,
) -> tuple[float, float]:
    """
...
def analyze_game(game: dict, _agent_cfg: dict | None = None) -> list[dict]:
    ...
    if home_fip and home_ip < _IP_FULL_CONFIDENCE:
        ...
        home_fip = round(_hconf * home_fip + (1.0 - _hconf) * config.MLB_LEAGUE_AVG_FIP, 2)
    if away_fip and away_ip < _IP_FULL_CONFIDENCE:
        _aconf  = away_ip / _IP_FULL_CONFIDENCE
        away_fip = round(_aconf * away_fip + (1.0 - _aconf) * config.MLB_LEAGUE_AVG_FIP, 2)
    ...
    def _build_pick(direction: str, edge: float, our_prob: float, fair_mkt: float) -> dict:
        ...
        away_hr_fb_note = ""
        home_hr_fb_note = ""
        if away_hr_fb is not None and away_hr_fb > config.MLB_LEAGUE_HR_PER_FB * 1.4:
            away_hr_fb_note = f" HR/FB={away_hr_fb:.1%} [alto, xFIP < FIP]"
        elif away_hr_fb is not None and away_hr_fb < config.MLB_LEAGUE_HR_PER_FB * 0.6:
            away_hr_fb_note = f" HR/FB={away_hr_fb:.1%} [bajo, xFIP > FIP]"
        if home_hr_fb is not None and home_hr_fb > config.MLB_LEAGUE_HR_PER_FB * 1.4:
            home_hr_fb_note = f" HR/FB={home_hr_fb:.1%} [alto, xFIP < FIP]"
        elif home_hr_fb is not None and home_hr_fb < config.MLB_LEAGUE_HR_PER_FB * 0.6:
...
def analyze_games_with_params(games: list[dict], params: ModelParams | None = None) -> dict:
    """
    Pipeline principal de análisis MLB con parámetros configurables.
    
    Args:
        games: lista de dicts del collector (get_todays_mlb_games).
        params: ModelParams con thresholds configurables. None = defaults.
    
    Returns:
        {
            "picks": [...],       # picks de totals (OVER/UNDER)
            "picks_ml": [...],    # picks de moneyline
            "picks_rl": [...],    # picks de run line
            "picks_f5": [...],    # picks de F5 innings
            "games_analyzed": int # total de partidos analizados
        }
    """
    ...
    _agent_cfg = {
        "min_edge":              p.min_edge,
        "min_edge_under":        p.min_edge_under,
        "min_prob_over":         p.min_prob_over,
        "min_prob_under":        p.min_prob_under,
    }
    ...
    for game in games:
        try:
            picks = analyze_game(game, _agent_cfg)
            for pick in picks:
                bt = pick.get("bet_type", "TOTAL")
                if bt == "TOTAL":
                    all_picks.append(pick)
                elif bt == "ML":
                    all_picks_ml.append(pick)
                elif bt == "RL":
                    all_picks_rl.append(pick)
                elif bt == "F5":
                    all_picks_f5.append(pick)
        except Exception:
            pass
    
    all_picks.sort(key=lambda x: x["edge"], reverse=True)
    all_picks_ml.sort(key=lambda x: x["edge"], reverse=True)
    all_picks_rl.sort(key=lambda x: x["edge"], reverse=True)
    all_picks_f5.sort(key=lambda x: x["edge"], reverse=True)
    
    return {
        "picks":           all_picks[:p.max_picks],
        "picks_ml":        all_picks_ml[:p.ml_max_picks],
        "picks_rl":        all_picks_rl[:p.rl_max_picks],
        "picks_f5":        all_picks_f5[:3],
...
def analyze_live_game(game: dict, live_state: dict, pitcher_state: dict,
    ...
    home_p        = game.get("home_pitcher") or {}
    away_p        = game.get("away_pitcher") or {}
    home_fip_live = float(home_p.get("fip") or config.MLB_LEAGUE_AVG_FIP)
    away_fip_live = float(away_p.get("fip") or config.MLB_LEAGUE_AVG_FIP)
    ...
    home_ops    = float(game.get("home_ops") or config.MLB_LEAGUE_AVG_OPS)
    away_ops    = float(game.get("away_ops") or config.MLB_LEAGUE_AVG_OPS)
    ...
    sigma_remaining = config.MLB_RUN_TOTAL_SIGMA * (innings_remaining_frac ** 0.5)
...
def _preprocess_game_for_agent(game: dict, cfg: dict) -> dict:
    ...
    for side in ("home", "away"):
        ops_s  = g.get(f"{side}_ops_season")  or g.get(f"{side}_ops") or config.MLB_LEAGUE_AVG_OPS
        ...
        xfip_s = p.get("xfip")        or p.get("fip")        or config.MLB_LEAGUE_AVG_FIP
...
def _zeus_extra_filter(picks: list[dict], game: dict, cfg: dict) -> list[dict]:
    ...
    home_p = game.get("home_pitcher") or {}
    away_p = game.get("away_pitcher") or {}
    home_xfip = home_p.get("xfip") or home_p.get("fip") or config.MLB_LEAGUE_AVG_FIP
    away_xfip = away_p.get("xfip") or away_p.get("fip") or config.MLB_LEAGUE_AVG_FIP

    home_weak  = home_xfip > config.MLB_LEAGUE_AVG_FIP + ov_delta
    away_weak  = away_xfip > config.MLB_LEAGUE_AVG_FIP + ov_delta
    home_elite = home_xfip < config.MLB_LEAGUE_AVG_FIP - un_delta
    away_elite = away_xfip < config.MLB_LEAGUE_AVG_FIP - un_delta
        ...
        if direction == "OVER":
            ...
            if home_weak and away_weak:
                p["zeus_reason"] = (
                    ...
                    f"visitante xFIP {away_xfip:.2f} — ambos > {config.MLB_LEAGUE_AVG_FIP + ov_delta:.2f}"
                )
                result.append(p)
            else:
                p["zeus_skip"] = (
                    f"Zeus requiere AMBOS pitchers débiles (xFIP > {config.MLB_LEAGUE_AVG_FIP + ov_delta:.2f}) "
            ...
            if home_elite and away_elite:
                p["zeus_reason"] = (
                    ...
                    f"visitante xFIP {away_xfip:.2f} — ambos < {config.MLB_LEAGUE_AVG_FIP - un_delta:.2f}"
                )
                result.append(p)
            else:
                p["zeus_skip"] = (
                    f"Zeus requiere AMBOS pitchers élite (xFIP < {config.MLB_LEAGUE_AVG_FIP - un_delta:.2f}) "
...
def _atena_extra_filter(picks: list[dict], game: dict, cfg: dict) -> list[dict]:
    ...
    for p in picks:
        direction = p.get("direction", "")
        home_ops_s = game.get("home_ops_season") or game.get("home_ops") or config.MLB_LEAGUE_AVG_OPS
        home_ops_r = game.get("home_ops_recent") or home_ops_s
        away_ops_s = game.get("away_ops_season") or game.get("away_ops") or config.MLB_LEAGUE_AVG_OPS
...
def _narrative_mlb(pick: dict) -> str:
    ...
    # Stats de pitchers
    home_fip      = pick.get("home_fip")   or config.MLB_LEAGUE_AVG_FIP
    away_fip      = pick.get("away_fip")   or config.MLB_LEAGUE_AVG_FIP
    ...
    # OPS ofensivos
    home_ops      = pick.get("home_ops") or config.MLB_LEAGUE_AVG_OPS
    away_ops      = pick.get("away_ops") or config.MLB_LEAGUE_AVG_OPS
    ...
    _WEAK   = config.MLB_LEAGUE_AVG_FIP + 0.50   # xFIP "débil" → favorece carreras
    _STRONG = config.MLB_LEAGUE_AVG_FIP - 0.75   # xFIP "élite" → suprime carreras
    ...
    if direction == "OVER":
        ...
        elif home_eff > config.MLB_LEAGUE_AVG_FIP + 0.2 or away_eff > config.MLB_LEAGUE_AVG_FIP + 0.2:
        ...
        elif home_ops < config.MLB_LEAGUE_AVG_OPS - 0.020 and away_ops < config.MLB_LEAGUE_AVG_OPS - 0.020:
            parts.append(
