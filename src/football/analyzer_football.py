...
def analyze_match(game: dict, home_stats: dict, away_stats: dict, opts: dict | None = None) -> dict:
    """
    ...
    opts (opcional):
      min_edge, min_prob_win, min_prob_draw, min_prob_ou, min_prob_btts,
    ...
    min_edge       = opts.get("min_edge") or getattr(config, "FOOTBALL_MIN_EDGE", config.MIN_EDGE)
    min_prob_win   = opts.get("min_prob_win",  _MIN_PROB_WIN)
    min_prob_draw  = opts.get("min_prob_draw", _MIN_PROB_DRAW)
    min_prob_ou    = opts.get("min_prob_ou",   _MIN_PROB_OU)
    min_prob_btts  = opts.get("min_prob_btts", _MIN_PROB_BTTS)

    allow_win_set   = "allow_win" in opts
    allow_draw_set  = "allow_draw" in opts
    allow_over_set  = "allow_over" in opts
    allow_under_set = "allow_under" in opts
    allow_btts_set  = "allow_btts" in opts

    allow_win   = opts["allow_win"]   if allow_win_set   else True
    allow_draw  = opts["allow_draw"]  if allow_draw_set  else True
    allow_over  = opts["allow_over"]  if allow_over_set  else True
    allow_under = opts["allow_under"] if allow_under_set else True
    allow_btts  = opts["allow_btts"]  if allow_btts_set  else True
    ...
    # ── Evaluar 1X2 ──────────────────────────────────────────────────────────
    candidates_12x = [
        (probs["p_home"], impl_home, game["h2h_home"], f"{home} (LOCAL)",    min_prob_win),
        (probs["p_draw"], impl_draw, game["h2h_draw"], "EMPATE",             min_prob_draw),
        (probs["p_away"], impl_away, game["h2h_away"], f"{away} (VISITANTE)",min_prob_win),
    ]

    for our_prob, impl_prob, odds, label, min_prob in candidates_12x:
        if odds is None:
            continue
        if "LOCAL" in label and not allow_win:
            continue
        if "VISITANTE" in label and not allow_win:
            continue
        if "EMPATE" in label and not allow_draw:
            continue
    ...
    if game.get("total_over") and game.get("total_under"):
        ...
        for our_prob, impl_prob, odds, label in [
            ...
            if "Over" in label and not allow_over:
                continue
            if "Under" in label and not allow_under:
                continue
            edge = our_prob - impl_prob
            if our_prob >= min_prob_ou and edge >= min_edge:
                picks.append(_build_pick(
                    game=game,
    ...
    if game.get("btts_yes") and game.get("btts_no"):
        ...
        for our_prob, impl_prob, odds, label in [
            ...
            if not allow_btts:
                continue
            edge = our_prob - impl_prob
            if our_prob >= min_prob_btts and edge >= min_edge:
                picks.append(_build_pick(
                    game=game,
                    bet_type="BTTS",
                    selection=label,
