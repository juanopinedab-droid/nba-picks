...
Ejecutar: python cli/picks_football.py

Marca resultados:  python cli/picks_football.py --resultado <ID> WIN
Ver pendientes:    python cli/picks_football.py --pendientes
Ver historial:     python cli/picks_football.py --historial
Filtrar partido:   python cli/picks_football.py --partido Arsenal
"""

import argparse
from datetime import date

from src.core import database, config, bankroll
from src.football import collector_football as collector
from src.football import analyzer_football as analyzer
from src.service.picks_football import execute as run_pipeline
...
def run_football_picks(partido: str | None = None):
    """Pipeline principal de futbol: datos → modelo → output."""
    print_header()

    from src.core.types import JobContext
    ctx = JobContext(job_id="cli_football")
    ...
    if not has_any_pick:
        ...
        print(f"       (minimo requerido: {getattr(config, 'FOOTBALL_MIN_EDGE', config.MIN_EDGE):.0%})")
